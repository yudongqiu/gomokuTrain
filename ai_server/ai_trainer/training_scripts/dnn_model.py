        ...
        for epoch in range(epochs):
            try:
                ...
                if v_size > 0:
                    ...
                    status_report = f"Epoch {epoch:5d}/{epochs} {t_size:9d}/{t_size} | loss {average_loss:.5f} | val_loss {valid_loss:.5f} | {time.time() - t_start:.2f}s"
                    print(status_report)
                    with open('status_report.txt', 'a') as f:
                        f.write(status_report + '\n')
                else:
                    status_report = f"Epoch {epoch:5d}/{epochs} {t_size:9d}/{t_size} | loss {average_loss:.5f} | {time.time() - t_start:.2f}s"
                    with open('status_report.txt', 'a') as f:
                        f.write(status_report + '\n')
...
def get_new_model():
    return DNNModel(n_stages=4, planes=168, kernel_size=3)
